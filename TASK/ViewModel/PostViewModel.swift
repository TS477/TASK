...
class PostViewModel: ObservableObject {
    ...
    func loadMorePosts() async {
        do {
            // 計算正確的 offset: page * pageSize
            let offset = currentPage * PAGE_SIZE
            
            // 獲取新的帖子
            let newPosts = try await fetchPosts(offset: offset)
    ...
    // 實際的網絡請求函數
    private func fetchPosts(offset: Int) async throws -> [PostModel] {
